...
export function createDiagnosticCapture() {
  ...
  const finishLine = () => {
    // A newline completes any pending UTF8 sequence. Otherwise withhold a
    // split final character from a capped prefix or an unterminated last line.
    const text = prefix.at(-1) === 10 ? prefix.toString('utf8') : new StringDecoder('utf8').write(prefix);
    const diagnostic = isDiagnosticLine(text);
    if (!full() && (diagnostic || following > 0)) {
      following = diagnostic ? 2 : following - 1;
  ...
  return {
    ...
    push(chunk) {
      ...
      while (start < chunk.length && !full()) {
        ...
        if (!lineBytes && !following && (chunk[start] === 10 || (chunk[start] === 13 && chunk[start + 1] === 10))) {
          const blankStart = start;
          do { start += chunk[start] === 13 ? 2 : 1; }
          while (start < chunk.length && (chunk[start] === 10 || (chunk[start] === 13 && chunk[start + 1] === 10)));
          offset += start - blankStart;
          continue;
        }
        const newline = chunk.indexOf(10, start);
        const end = newline < 0 ? chunk.length : newline + 1;
        // Complete, unretained lines need no owned buffer or line state.
        if (!lineBytes && !following && newline >= 0 && end - start <= DIAGNOSTIC_BYTES + 4 && !isDiagnosticLine(chunk.toString('utf8', start, end))) {
          offset += end - start;
          start = end;
          continue;
        }
        const keep = chunk.subarray(start, Math.min(end, start + DIAGNOSTIC_BYTES + 4 - prefix.length));
        // Borrow only until finishLine returns below; incomplete and retained
        // evidence owns its bytes so later chunk reuse cannot change it.
        if (keep.length) prefix = !lineBytes && newline >= 0 ? keep : Buffer.concat([prefix, keep]);
        lineBytes += end - start;
        start = end;
