...
describe('adaptive reducer', () => {
  ...
  test('invalid UTF8 before, inside and after a diagnosis preserves raw omissions and final evidence', () => {
    const before = Buffer.from([0xff, 0xfe, 10]);
    const diagnosis = Buffer.concat([Buffer.from('FAIL invalid '), Buffer.from([0xc0, 0xaf]), Buffer.from(' evidence\n')]);
    const context = Buffer.concat([Buffer.from('after '), Buffer.from([0xff]), Buffer.from(' bytes\ncontext\n')]);
    const passing = 'passed unrelated check\n';
    const tail = Buffer.concat([Buffer.from('final '), Buffer.from([0xf0, 0x9f]), Buffer.from(' sentinel\n')]);
    const raw = Buffer.concat([before, diagnosis, context, Buffer.from(passing.repeat(700)), tail]);
    const result = reduceOutput({ raw, text: raw.toString('utf8'), code: 7, logPath: '/private/log' });
    expect(result.compacted).toBe(true);
    expect(result.text).toContain(diagnosis.toString('utf8'));
    expect(result.text).toContain(tail.toString('utf8'));
    expect(result.text.split('\n')[0]).toContain('text_lossy=true');
    const retained = before.length + diagnosis.length + context.length + Buffer.byteLength(passing.repeat(23)) + tail.length;
    expect(result.sourceBytes).toBe(raw.length);
    expect(result.omittedBytes).toBe(raw.length - retained);
    expect(result.outputBytes).toBe(Buffer.byteLength(result.text));
  });
  test('replacement-character expansion never changes the raw tail endpoint', () => {
    const raw = Buffer.concat([Buffer.from('FAIL early\n'), Buffer.alloc(50000, 255), Buffer.from('\nfinal sentinel\n')]);
    const result = reduceOutput({ raw, text: raw.toString('utf8'), code: 7, logPath: '/private/log' });
    expect(result.compacted).toBe(true);
    expect(result.text).toContain('FAIL early');
    expect(result.text).toContain('final sentinel');
    expect(result.text.split('\n')[0]).toContain('text_lossy=true');
    expect(result.omittedBytes).toBe(raw.length - 4096);
    const short = Buffer.from([255, 0]);
    const passthrough = reduceOutput({ raw: short, text: short.toString('utf8'), code: 7, logPath: '/private/log' });
    expect(passthrough.compacted).toBe(false);
    expect(passthrough.outputBytes).toBe(short.length);
    expect(passthrough.omittedBytes).toBe(0);
  });
});

describe('bounded streaming diagnostics', () => {
  test('chunk boundaries and later buffer reuse cannot change retained evidence', () => {
    const input = Buffer.from('ordinary\r\n\x1b[31mFAIL\x1b[0m é😀 mismatch\nExpected: 17\nReceived: 12\ncontext one\ncontext two\nordinary\n');
    const whole = createDiagnosticCapture();
    whole.push(input);
    const expected = whole.finish();
    for (let split = 1; split < input.length; split++) {
      const capture = createDiagnosticCapture();
      for (const part of [input.subarray(0, split), input.subarray(split)]) {
        const reusable = Buffer.from(part);
        capture.push(reusable);
        reusable.fill(0);
      }
      expect(capture.finish()).toEqual(expected);
      expect(capture.bufferedBytes).toBeLessThanOrEqual(DIAGNOSTIC_BYTES * 2 + 4);
    }
  });
  test('capped and unterminated UTF8 prefixes withhold only split characters', () => {
    for (const ending of [Buffer.from([0xc3]), Buffer.from([0xf0, 0x9f, 0x98])]) {
      const capture = createDiagnosticCapture();
      capture.push(Buffer.concat([Buffer.from('FAIL valid é😀 '), ending]));
      expect(capture.finish()[0]!.bytes.toString()).toBe('FAIL valid é😀 ');
    }
  });
  ...
  test('CRLF blank runs and split CRLF keep exact offsets and following context', () => {
    const capture = createDiagnosticCapture();
    const prelude = '\r\n'.repeat(2048) + '\n\r\n';
    capture.push(Buffer.from(prelude.slice(0, -1)));
    capture.push(Buffer.from('\nFAIL after CRLF\r\n\r'));
    capture.push(Buffer.from('\n\r\n' + '\r\n'.repeat(2048)));
    const ranges = capture.finish();
    expect(ranges[0]!.start).toBe(Buffer.byteLength(prelude));
    expect(Buffer.concat(ranges.map(range => range.bytes)).toString()).toBe('FAIL after CRLF\r\n\r\n\r\n');
    expect(ranges.map(range => range.start)).toEqual([prelude.length, prelude.length + 'FAIL after CRLF\r\n'.length, prelude.length + 'FAIL after CRLF\r\n\r\n'.length]);
  });
  ...
  test('a full early evidence budget preserves a later diagnosis and the baseline tail', () => {
    const early = 'FAIL early marker '.padEnd(DIAGNOSTIC_BYTES - 1, 'x') + '\n';
    const late = 'FAIL later marker\nExpected later details\n';
    const source = Buffer.from(early + 'passed check\n'.repeat(25000) + late + 'passed check\n'.repeat(1000) + 'final sentinel\n');
    const capture = createDiagnosticCapture();
    capture.push(source);
    const diagnostics = capture.finish();
    const raw = source.subarray(source.length - CAPTURE_BYTES);
    const input = { raw, text: raw.toString('utf8'), code: 7, logPath: '/private/log', outputBytes: source.length, captureTruncated: true };
    const baseline = reduceOutput(input);
    expect(baseline.text).toContain(late);
    const retained = reduceOutput({ ...input, diagnostics });
    expect(retained.text).toContain(early);
    expect(retained.text).toContain(late);
    expect(retained.text).toContain('final sentinel');
    expect(diagnostics.reduce((sum, range) => sum + range.bytes.length, 0)).toBe(DIAGNOSTIC_BYTES);
    expect(retained.omittedBytes).toBe(baseline.omittedBytes - DIAGNOSTIC_BYTES);
    // Expected restarts the following-context window, retaining two passing
    // lines after the pair of diagnoses, plus two lines before it.
    expect(retained.omittedBytes).toBe(source.length - DIAGNOSTIC_BYTES - Buffer.byteLength(late + 'passed check\n'.repeat(4 + 23) + 'final sentinel\n'));
  });
  test('early diagnostics never shrink the 4 KiB baseline tail and total evidence stays within 6 KiB', () => {
    const early = 'FAIL early marker '.padEnd(DIAGNOSTIC_BYTES - 1, 'x') + '\n';
    const tail = 'tail beginning marker\n' + 'y'.repeat(4050) + '\nfinal sentinel\n';
    const source = Buffer.from(early + 'passed check\n'.repeat(25000) + tail);
    const capture = createDiagnosticCapture();
    capture.push(source);
    const raw = source.subarray(source.length - CAPTURE_BYTES);
    const input = { raw, text: raw.toString('utf8'), code: 7, logPath: '/private/log', outputBytes: source.length, captureTruncated: true };
    const baseline = reduceOutput(input);
    const retained = reduceOutput({ ...input, diagnostics: capture.finish() });
    expect(baseline.text).toContain(tail);
    expect(retained.text).toContain(tail);
    expect(retained.text).toContain(early);
    expect(retained.omittedBytes).toBe(baseline.omittedBytes - DIAGNOSTIC_BYTES);
    expect(source.length - retained.omittedBytes).toBe(6144);
  });
  ...
  test('invalid UTF8 diagnostics crossing any raw cutoff remain whole and counted once', () => {
    const diagnosis = Buffer.concat([Buffer.from('FAIL boundary '), Buffer.from([255, 128]), Buffer.from(' é😀 mismatch\n')]);
    const passing = 'unrelated check passed\n';
    const tail = Buffer.concat([Buffer.from('final '), Buffer.from([255]), Buffer.from(' sentinel\n')]);
    const source = Buffer.concat([diagnosis, Buffer.from(passing.repeat(400)), tail]);
    for (let cutoff = 1; cutoff < diagnosis.length; cutoff++) {
      const raw = source.subarray(cutoff);
      const result = reduceOutput({ raw, text: raw.toString('utf8'), code: 7, logPath: '/private/log', outputBytes: source.length, captureTruncated: true, diagnostics: [{ start: 0, bytes: diagnosis }] });
      expect(result.text).toContain(diagnosis.toString('utf8'));
      expect(result.text.match(/FAIL boundary/g)?.length).toBe(1);
      expect(result.text).toContain(tail.toString('utf8'));
      expect(result.text.split('\n')[0]).toContain('text_lossy=true');
      expect(result.omittedBytes).toBe(source.length - diagnosis.length - Buffer.byteLength(passing.repeat(23)) - tail.length);
    }
  });
});
...
describe('one-shot argv execution', () => {
  ...
  test('binary capture keeps complete private bytes and a labelled early diagnosis and final tail', async () => {
    const script = 'process.stdout.write(Buffer.concat([Buffer.from("FAIL early "),Buffer.from([255]),Buffer.from(" diagnosis\\ncontext\\ncontext\\n"),Buffer.alloc(300000,255),Buffer.from("\\nfinal "),Buffer.from([128]),Buffer.from(" sentinel\\n")]));process.exitCode=7';
    const result = await check({ argv: [node, '-e', script], logPath: path('binary-long.log') });
    const complete = readFileSync(result.logPath);
    expect(result.code).toBe(7);
    expect(result.captureTruncated).toBe(true);
    expect(result.raw).toEqual(complete.subarray(complete.length - CAPTURE_BYTES));
    expect(result.outputBytes).toBe(complete.length);
    expect(result.rendered.text).toContain('FAIL early � diagnosis');
    expect(result.rendered.text).toContain('final � sentinel');
    expect(result.rendered.text.split('\n')[0]).toContain('text_lossy=true');
    const early = Buffer.byteLength('FAIL early ') + 1 + Buffer.byteLength(' diagnosis\ncontext\ncontext\n');
    expect(result.rendered.omittedBytes).toBe(complete.length - early - 4096);
    expect(result.stdout).toEqual(Buffer.from(result.rendered.text));
  });
